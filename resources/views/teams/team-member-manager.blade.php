            ...
            <x-form-section submit="addTeamMember">
                ...
                <x-slot name="form">
                    ...
                    <div class="col-span-6 sm:col-span-4">
                        ...
                        <x-input id="email" type="email" class="mt-1 block w-full form-control" wire:model="addTeamMemberForm.email" />
                                        ...
                                        <div class="{{ isset($addTeamMemberForm['role']) && $addTeamMemberForm['role'] !== $role->key ? 'opacity-50' : '' }}">
                                            ...
                                            <div class="flex items-center">
                                                <div class="text-sm text-gray-600 dark:text-gray-400 {{ $addTeamMemberForm['role'] == $role->key ? 'font-semibold' : '' }}">
                                                    {{ strtolower($role->name)=='editor'?'Team Member':$role->name }}
                                                </div>
                                ...
                                <div class="flex items-center">
                                    ...
                                    @if (Gate::check('updateTeamMember', $team) && Laravel\Jetstream\Jetstream::hasRoles())
                                        <button class="ml-2 text-sm text-gray-400 underline" wire:click="manageRole('{{ $user->id }}')">
                                            {{ strtolower(Laravel\Jetstream\Jetstream::findRole($user->membership->role)->name)=='editor'?'Team Member':Laravel\Jetstream\Jetstream::findRole($user->membership->role)->name}}
                                        </button>
