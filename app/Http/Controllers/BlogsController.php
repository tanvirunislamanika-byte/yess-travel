...
class BlogsController extends Controller
{
    public function index(Request $request){
        $data = ModulesData::where('module_id', 23)->where('status','active');
        ...
        $arr['blogs'] = $data->paginate(10);

        $arr['recent_data'] = ModulesData::where('module_id', 23)->where('status','active')->orderBy('id','desc')->take(3)->get(); 
        $arr['archives'] = $this->lastThreeMonths();
        return view('blogs.index')->with($arr);
    }   
    ...
    public function detail($slug){
    	...
    	$data['recent_data'] = ModulesData::where('module_id', 23)->where('id','!=',$data['blog']->id)->where('status','active')->orderBy('id','desc')->take(3)->get(); 
