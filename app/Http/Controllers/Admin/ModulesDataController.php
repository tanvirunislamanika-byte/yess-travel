...
class ModulesDataController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($request->cusines) && is_array($request->cusines)) {
            $data->cusines = json_encode($request->cusines);
        }
   ...
   public function fetchModulesData(Request $request)
    {
        ...
        if (auth()->user()->hasRole('admin')) {
            ...
            $modulesDataQuery = ModulesData::where('module_id', $module->id)->where('user_id',auth()->user()->id)->orderBy('id','DESC');
        }
